...
type SBundlePool struct {
	...
	istanbul      atomic.Bool
	eip2718       atomic.Bool
	eip1559       atomic.Bool
	shanghai      atomic.Bool
	currentMaxGas atomic.Uint64
}
...
func (p *SBundlePool) validateTx(tx *types.Transaction) error {
	...
	if !p.eip2718.Load() && tx.Type() != types.LegacyTxType {
		return core.ErrTxTypeNotSupported
	}
	// Reject dynamic fee transactions until EIP-1559 activates.
	if !p.eip1559.Load() && tx.Type() == types.DynamicFeeTxType {
		return core.ErrTxTypeNotSupported
	}
	...
	if p.shanghai.Load() && tx.To() == nil && len(tx.Data()) > params.MaxInitCodeSize {
	...
	if p.currentMaxGas.Load() < tx.Gas() {
		return ErrGasLimit
	}
